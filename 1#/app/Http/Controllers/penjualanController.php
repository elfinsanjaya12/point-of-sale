        ...
        if ($user->hasRole('penjaga') || $user->hasRole('pemilik')) {
          ...
          if ($request->has('simpanpenjualan')) {             
            ...
            foreach ($penjualanItems as $value) {
              ...
              $cek_barang_persediaan = BarangPersediaan::where('id_barang', $value->id_barang)->first();              
              // dd($cek_barang_persediaan);
              if (count($cek_barang_persediaan) > 0) {                
                $cek_barang_persediaan->tanggal_jual = $penjualans->tanggal;
                $cek_barang_persediaan->stok_jual += $value->jumlah_jual;
                $cek_barang_persediaan->save();  
              } else {
